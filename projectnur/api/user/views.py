...
#validation part
    ...
    if len(password) < 5:
        return JsonResponse({'error': 'Password must be atleast of 5 char'})


    UserModel = get_user_model()

    try:
        user = UserModel.objects.get(email_id=username)

        if user.check_password(password):
            usr_dict = UserModel.objects.filter(email_id=username).values().first()
        ...
        return JsonResponse({'error': 'Inavalid Email'})
